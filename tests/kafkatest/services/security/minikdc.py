...
class MiniKdc(KafkaPathResolverMixin, Service):
    ...
    def start_node(self, node):
        ...
        cmd += " %s kafka.security.minikdc.MiniKdc %s %s %s %s 1>> %s 2>&1 &" % (self.path.script("kafka-run-class.sh", node), MiniKdc.WORK_DIR, MiniKdc.PROPS_FILE, MiniKdc.KEYTAB_FILE, principals, MiniKdc.LOG_FILE)
        ...
        with node.account.monitor_log(MiniKdc.LOG_FILE) as monitor:
            ...
            monitor.wait_until("MiniKdc Running", timeout_sec=300, backoff_sec=5, err_msg="MiniKdc didn't finish startup")
